...
public class AddOrEditReferencePointActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        String xx = new DecimalFormat("##.##", symbols).format((x/50)-4f);
        String yy = new DecimalFormat("##.##", symbols).format((y/50)-6.2f);
        if (x/50 > 4 & x/50 < 17){
            if(y/50<33 & y/50>6.5){
                if(y/50 > 16 || x/50>6.2){
                    if (y/50 < 22.7 || x/50>6.2){
                        if (y/50 > 17.7 || x/50<14.7) {
                            if (y / 50 < 22 || x / 50 < 14.7) {
                                etRpX.setText(xx);
                                etRpY.setText(yy);
                                lay_loc.setY(y-300);
                                lay_loc.setX(x-50);
